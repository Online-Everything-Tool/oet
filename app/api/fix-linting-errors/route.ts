...
if (!API_KEY) {
  console.error(
    'FATAL ERROR (fix-linting-errors): GEMINI_API_KEY missing.'
  );
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('[API fix-linting-errors] Prompt template load error:', errMsg, templateError);
    return NextResponse.json(
      {
        success: false,
  ...
  for (const file of filesToFix) {
    ...
    // console.log(`[API fix-linting-errors] Populated prompt for ${file.path}:\n${populatedPrompt}`); // Potentially very verbose
    ...
    try {
      ...
      console.log(`[API fix-linting-errors] Raw AI response for ${file.path} (length ${rawResponseText.length}):\n---START RAW---\n${rawResponseText}\n---END RAW---`);
      
      const markdownFenceRegex = /^```(?:typescript|javascript|tsx|jsx)?\s*[\r\n]?|\s*[\r\n]?```$/g;
      const strippedResponseText = rawResponseText.replace(markdownFenceRegex, '').trim();
      
      if (rawResponseText !== strippedResponseText) {
        console.log(`[API fix-linting-errors] Markdown fences stripped from AI response for ${file.path}. New length: ${strippedResponseText.length}`);
      } else {
        console.log(`[API fix-linting-errors] No Markdown fences found or stripped for ${file.path}.`);
      }
      
      if (!strippedResponseText || strippedResponseText === file.currentContent) {
         console.warn(`[API fix-linting-errors] AI did not change content for ${file.path} or returned empty (after stripping fences). Original content will be used.`);
        ...
        console.log(`[API fix-linting-errors] AI PROPOSED FIXES for ${file.path}.`);
      }

    } catch (error: unknown) {
      const extractedMessage =
        error instanceof Error ? error.message : `Unknown AI service error for ${file.path}.`;
      
      console.error(
        `[API fix-linting-errors] Error during AI analysis for ${file.path}: ${extractedMessage}`,
        error // Log the full error object too
      );
      ...
      allSucceededOverall = false; 
       ...
       if (
        ...
        ((error as { message: string }).message.includes('response was blocked due to safety') || 
         (error as { message: string }).message.includes('Candidate was blocked due to safety'))
       ) {
  ...
  if (!allSucceededOverall && Object.values(fixedFilesResults).every(content => content === null)) {
    overallMessage = 'AI lint fixing failed for all files due to errors or safety blocks.';
    ...
    overallMessage = 'Lint fixing completed with some errors. Not all files could be processed.';
    console.warn(`[API fix-linting-errors] ${overallMessage}`);
  } else if (allSucceededOverall && !anyFileActuallyFixedByAI) {
    overallMessage = 'Lint fixing process completed. AI proposed no changes to any files (after stripping fences).';
    console.log(`[API fix-linting-errors] ${overallMessage}`);
  } else if (allSucceededOverall && anyFileActuallyFixedByAI) {
    overallMessage = 'Lint fixing process completed successfully. AI proposed fixes for one or more files (fences stripped).';
    console.log(`[API fix-linting-errors] ${overallMessage}`);
  }
