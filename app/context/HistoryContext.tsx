...
export const HistoryProvider = ({ children }: HistoryProviderProps) => {
  ...
  useEffect(() => {
    ...
    setIsHistoryEnabled(loadedEnabledState); setToolPreferences(loadedPrefs); setIsSettingsLoaded(true);
    console.log("[HistoryCtx] Settings loaded from localStorage.", { isHistoryEnabled: loadedEnabledState, toolPreferences: loadedPrefs });
  }, []);
  ...
  const loadHistoryFromDb = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[HistoryCtx] Loaded ${loadedHistory.length} history entries from 'history' table.`);
      setIsHistoryLoaded(true);
    } catch (error) {
      console.error("[HistoryCtx] Error loading history from Dexie 'history' table:", error);
  ...
  const addHistoryEntry = useCallback(async (entryData: NewHistoryData): Promise<void> => {
    ...
    if (preference === 'off') {
      console.log(`[HistoryCtx] Skipping history add for ${toolRoute} due to 'off' preference.`);
    ...
    try {
      ...
      const existingEntriesForTool = await db.history.where('toolRoute').equals(toolRoute).toArray();
      const existingEntry = existingEntriesForTool.find(entry => areInputsEqual(entryData.input, entry.input));
      ...
      if (existingEntry) {
        ...
        console.log(`[HistoryCtx] Updated existing history entry ${existingEntry.id} for tool ${toolRoute}.`);
      } else {
        // Add new entry
        ...
        console.log(`[HistoryCtx] Added new history entry ${newEntry.id} for tool ${toolRoute}.`);
        ...
        if (currentCount > MAX_HISTORY_ENTRIES) {
          ...
          const oldestEntries = await db.history.orderBy('eventTimestamp').limit(excessCount).primaryKeys();
          await db.history.bulkDelete(oldestEntries);
          console.log(`[HistoryCtx] Pruned ${oldestEntries.length} oldest history entries.`);
        }
      }
  ...
  }, [isSettingsLoaded, isHistoryEnabled, getToolLoggingPreference, loadHistoryFromDb]); // Dependencies
  ...
  const deleteHistoryEntry = useCallback(async (idToDelete: string): Promise<void> => {
    ...
    try {
      ...
      console.error(`[HistoryCtx] Error deleting history entry ${idToDelete}:`, error);
  ...
  }, [loadHistoryFromDb]); // Added loadHistoryFromDb dependency
  ...
  const clearHistoryForTool = useCallback(async (toolRoute: string): Promise<void> => {
    ...
    try {
      ...
      // Update local state optimistically or reload
      setHistory(prev => prev.filter(entry => entry.toolRoute !== toolRoute));
      console.log(`[HistoryCtx] Cleared history for tool ${toolRoute}.`);
    } catch (error) {
      console.error(`[HistoryCtx] Error clearing history for tool ${toolRoute}:`, error);
      const message = error instanceof Error ? error.message : "Unknown DB error";
