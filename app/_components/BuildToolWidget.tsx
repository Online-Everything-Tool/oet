...
export default function BuildToolWidget({ suggestedDirectives, modelNameUsed }: BuildToolWidgetProps) {
    return (
        <div className="p-4 md:p-6 border border-[rgb(var(--color-border-base))] rounded-lg bg-[rgb(var(--color-bg-component))] shadow-sm space-y-4">
            ...
            {suggestedDirectives && suggestedDirectives.length > 0 && (
                <div className="pt-4 border-t border-gray-200">
                    ...
                    {modelNameUsed && (
                       <p className="text-xs text-gray-400 mt-2">Suggestions generated using {modelNameUsed}</p>
                    )}
                </div>
            )}
        </div>
