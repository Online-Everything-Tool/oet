...
export default function Header() {
  ...
  const formatDirectiveTitle = (directive: string): string => {
      return directive.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
           ...
           {isToolPage && ( // Only render if on a /tool/ page
             <div className="relative inline-block" ref={dropdownRef}>
                ...
                >
                     <span className="text-yellow-400" style={{ fontSize: '1.5rem' }} aria-hidden="true">
                ...
                {showFavoritesDropdown && (
                    <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50 py-1">
                        <div role="menu" aria-orientation="vertical" aria-labelledby="favorites-menu-button" tabIndex={-1}>
                            {favoritesLoaded && favorites.length > 0 ? (
                                ...
                                <p className="px-4 py-2 text-sm text-gray-500 italic">No favorites yet.</p>
                            )}
                        </div>
                    </div>
                )}
