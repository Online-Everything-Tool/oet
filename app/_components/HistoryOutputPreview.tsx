...
export default function HistoryOutputPreview({
  entry,
  metadata,
  ...
  const imageId = useMemo(() => {
    const id =
      outputConfig?.referenceType === 'imageLibraryId' &&
  ...
  useEffect(() => {
    ...
    const loadAndSetImage = async () => {
      ...
      try {
        ...
        if (!imageData) {
          console.warn(
            `[HistoryPreview] Image data not found for ID: ${imageId} (likely deleted).`
          );
          if (isActive) {
            setImageError('Image deleted');
            setImageUrl(null);
            setIsLoading(false);
          }
          return;
        ...
        if (imageData.thumbnailBlob) {
          ...
          throw new Error(
            'Image data retrieved, but blob/thumbnailBlob missing.'
          );
        }
  ...
  if (imageId) {
    if (isLoading) {
      return (
        <div className="w-full h-full bg-gray-200 rounded animate-pulse min-h-[40px]"></div>
      );
    }
    if (imageError) {
      return (
        <span
          className="text-xs text-red-500 italic flex items-center justify-center h-full text-center px-1 leading-tight"
          title={imageError}
        >
          {imageError}
        </span>
