...
export default function ImageFlipClient({ toolTitle, toolRoute }: ImageFlipClientProps) {
    ...
    const flip = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
        ...
        if (flipType === 'horizontal') { ctx.scale(-1, 1); ctx.translate(-w, 0); }
        else { ctx.scale(1, -1); ctx.translate(0, -h); }
    ...
    useEffect(() => {
        ...
        if (selectedFile?.blob) {
            ...
            } catch (e) { console.error("Error creating object URL:", e); setError("Could not create preview for selected image."); setOriginalImageSrc(null); setFileName(null); }
    ...
    useEffect(() => {
        if (selectedFile && originalImageSrc) {
            const triggerProcessing = async () => {
                ...
                const newImageId = await processImage( selectedFile, flip, 'auto', outputFileName, { flipType: flipType } );
    ...
    const handleFlipTypeChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => { setFlipType(event.target.value as 'horizontal' | 'vertical'); setIsCopied(false); }, []);
    const handleClear = useCallback(() => { setOriginalImageSrc(null); setProcessedImageSrc(null); setFileName(null); setFlipType('horizontal'); setError(null); setIsLoading(false); setSelectedFile(null); setLastProcessedImageId(null); setIsCopied(false); }, [setOriginalImageSrc, setProcessedImageSrc, setFileName, setError, setIsLoading]);
    const handleDownload = useCallback(() => { if (!processedImageSrc || !fileName) { setError('No processed image available to download.'); return; } setError(null); const link = document.createElement('a'); const baseName = fileName.substring(0, fileName.lastIndexOf('.')) || fileName; const extension = fileName.substring(fileName.lastIndexOf('.') + 1) || 'png'; link.download = `flipped-${flipType}-${baseName}.${extension}`; link.href = processedImageSrc; document.body.appendChild(link); link.click(); document.body.removeChild(link); }, [processedImageSrc, fileName, flipType, setError]);
    const handleCopyToClipboard = useCallback(async () => { if (!lastProcessedImageId) { setError('No processed image available to copy.'); return; } setIsCopied(false); setError(null); try { const imageData = await getImage(lastProcessedImageId); if (!imageData?.blob) { throw new Error("Processed image data or blob not found."); } if (!navigator.clipboard?.write) { throw new Error("Clipboard API (write) not available."); } const clipboardItem = new ClipboardItem({ [imageData.blob.type]: imageData.blob }); await navigator.clipboard.write([clipboardItem]); setIsCopied(true); setTimeout(() => setIsCopied(false), 2000); } catch (err) { console.error('Failed to copy image to clipboard:', err); const message = err instanceof Error ? err.message : 'Unknown clipboard error'; setError(`Copy failed: ${message}`); } }, [lastProcessedImageId, getImage, setError]);
    ...
    return (
        <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
            ...
            <div className="flex flex-wrap gap-4 items-center p-3 rounded-md bg-[rgb(var(--color-bg-subtle))] border border-[rgb(var(--color-border-base))]">
                ...
                <fieldset className="flex gap-x-4 gap-y-2 items-center" disabled={isLoading || !originalImageSrc}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div className="space-y-1">
                    ...
                    <div className="w-full aspect-square border border-[rgb(var(--color-input-border))] rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
                        {originalImageSrc ? ( <Image src={originalImageSrc} alt={fileName || "Original"} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} /> ) : (<span className="text-sm text-[rgb(var(--color-input-placeholder))] italic">Select an image from library</span>)}
                    </div>
                </div>
                {/* Processed Image Preview */}
                 <div className="space-y-1">
                    ...
                    <div className="w-full aspect-square border border-[rgb(var(--color-input-border))] rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
                        ...
                        {!isLoading && processedImageSrc ? (<Image src={processedImageSrc} alt={fileName ? `Flipped ${fileName}` : "Flipped Image"} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} />) : !isLoading && (<span className="text-sm text-[rgb(var(--color-input-placeholder))] italic">Output appears here</span>)}
                    </div>
                </div>
            </div>
