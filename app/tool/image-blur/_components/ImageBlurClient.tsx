...
export default function ImageBlurClient({ toolRoute }: ImageBlurClientProps) {
  ...
  const generateDefaultOutputFilename = useCallback(() => {
    ...
    return `blurred-${baseName}-${toolState.blurAmount}.${extension}`;
  ...
  }, [getToolMetadata, toolState.blurAmount, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  const blurImage = useCallback(
    (ctx: CanvasRenderingContext2D, img: HTMLImageElement, options: { blurAmount: number }) => {
      const { blurAmount } = options;
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(
        ...
        blurImage,
        outputFileName,
        { blurAmount: debouncedBlurAmount },
        false // Always save as temporary initially
      );
    ...
    if (!toolState.processedFileId || toolState.processedFileId === null) {
      ...
      const triggerReprocessing = async () => {
        ...
        canvas.width = inputFile.blob.width;
        canvas.height = inputFile.blob.height;

        const ctx = canvas.getContext('2d');
        if (!ctx) throw new Error('Failed to get canvas context.');

        const img = new Image();
        img.src = URL.createObjectURL(inputFile.blob);

        img.onload = () => {
          blurImage(ctx!, img, { blurAmount: debouncedBlurAmount });
          const outputMimeType = inputFile.type === 'image/jpeg' ? 'image/jpeg' : 'image/png';
          const quality = outputMimeType === 'image/jpeg' ? 0.9 : undefined;

          canvas.toBlob(async (blob) => {
            if (!blob) throw new Error('Canvas toBlob failed.');
            await updateFileBlob(processedFile.id, blob, true);
            setProcessedImageSrcForUI(URL.createObjectURL(blob));
  ...
  const handleFilesSelectedFromModal = useCallback(
    ...
    [toolState.blurAmount, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, clearProcessingHookOutput, cleanupOrphanedTemporaryFiles]
  );
  ...
  const handleBlurAmountChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setState((prev) => ({ ...prev, blurAmount: newBlurAmount }));
  }, [setState]);
  ...
  }, [toolState.blurAmount, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  const canInitiateSaveCurrent = !!toolState.processedFileId && !processedOutputPermanent && !isProcessingImage && !isManuallySaving;
      ...
      <div className="flex flex-col gap-3 p-3 rounded-md bg-[rgb(var(--color-bg-subtle))] border border-[rgb(var(--color-border-base))]">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <Range
            ...
            min={0}
            max={50}
            disabled={isProcessingImage || isManuallySaving || !toolState.selectedFileId}
            showValue={true}
          />
        </div>
