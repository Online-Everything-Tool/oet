...
export default function ImageBlurClient({ toolRoute }: ImageBlurClientProps) {
  ...
  const [debouncedBlurAmount] = useDebounce(toolState.blurAmount, 500);
  ...
  const handleProcessIncomingSignal = useCallback(
    async (signal: IncomingSignal) => {
      ...
      if (
        ...
        setUiError(
          resolvedPayload.errorMessage || 'No transferable data received from source.'
        );
        return;
      }
  ...
  const blurDrawFunction = useCallback(
    (ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
      const { naturalWidth: w, naturalHeight: h } = img;
      ctx.filter = `blur(${debouncedBlurAmount}px)`;
  ...
  useEffect(() => {
    ...
    if (!toolState.processedFileId || toolState.processedFileId === null) {
      triggerProcessing();
    } else {
      const triggerReprocessing = async () => {
        const inputFile = await getFile(toolState.selectedFileId!);
        const processedFile = await getFile(toolState.processedFileId!);

        if (!inputFile?.blob || !processedFile) {
          setUiError('Original or processed image data not found for reprocessing.');
          return;
        }
        ...
        const result = await processImage(
          ...
          {},
          toolState.autoSaveProcessed,
          false // Do not create a permanent entry, update the existing one
        );

        if (result.blob && processedFile) {
          await updateFileBlob(processedFile.id, result.blob, true);
          const url = URL.createObjectURL(result.blob);
          setProcessedImageSrcForUI(url);
        }
      }
      triggerReprocessing();
    }
  ...
  const handleBlurAmountChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const newBlurAmount = parseInt(e.target.value, 10);
    if (!isNaN(newBlurAmount) && newBlurAmount >= 0 && newBlurAmount <= 100) {
