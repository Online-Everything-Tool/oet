...
export default function ImageBlurClient({ toolRoute }: ImageBlurClientProps) {
  ...
  const blurImage = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement, options: { blurAmount: number }) => {
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(inputFile, blurImage, outputFileName, { blurAmount: toolState.blurAmount }, false);
  ...
  }, [toolState.selectedFileId, toolState.blurAmount, toolState.processedFileId, isLoadingToolSettings, isProcessingImage, processImage, blurImage, getFile, setState, processingErrorHook, generateDefaultOutputFilename]);
  ...
  const handleBlurAmountChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newState: Partial<ImageBlurToolState> = {
    ...
    if (oldProcessedId) {
      cleanupOrphanedTemporaryFiles([oldProcessedId]).catch(e => console.error('[ImageBlur BlurAmountChange] Cleanup call failed:', e));
    }
  ...
  return (
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          <label className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">Blurred Image</label>
          <div className="w-full aspect-square border rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
