...
const DEFAULT_BLUR_TOOL_STATE: ImageBlurToolState = {
  blurAmount: 5,
...
export default function ImageBlurClient({ toolRoute }: ImageBlurClientProps) {
  const { state: toolState, setState, saveStateNow, isLoadingState: isLoadingToolSettings } = useToolState<ImageBlurToolState>(toolRoute, DEFAULT_BLUR_TOOL_STATE);
  ...
  const generateDefaultOutputFilename = useCallback(() => {
    ...
    return `blurred-${baseName}-blur-${toolState.blurAmount}.${extension}`;
  }, [originalFilenameForDisplay, processedImageSrcForUI, processedStoredFileForItde, toolState.blurAmount]);
  ...
  const handleProcessIncomingSignal = useCallback(async (signal: IncomingSignal) => {
    ...
    if (newSelectedFileId) {
      ...
      const newState: ImageBlurToolState = {
        selectedFileId: newSelectedFileId,
        processedFileId: null,
        blurAmount: toolState.blurAmount,
      ...
      if (destatedIds.length > 0) {
        cleanupOrphanedTemporaryFiles(destatedIds).catch(e => console.error('[ImageBlur ITDE Accept] Cleanup call failed:', e));
      }
    }

  }, [getToolMetadata, toolState.blurAmount, toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  const blurImage = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
    const { naturalWidth: w, naturalHeight: h } = img;
    ctx.filter = `blur(${toolState.blurAmount}px)`;
    ctx.drawImage(img, 0, 0, w, h);
  }, [toolState.blurAmount]);
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(inputFile, blurImage, outputFileName, {}, toolState.autoSaveProcessed);
      if (result.id) {
  ...
  }, [toolState.selectedFileId, toolState.blurAmount, toolState.processedFileId, toolState.autoSaveProcessed, isLoadingToolSettings, isProcessingImage, processImage, blurImage, getFile, setState, processingErrorHook, generateDefaultOutputFilename]);
  ...
  const handleFilesSelectedFromModal = useCallback(async (files: StoredFile[]) => {
    ...
    if (files?.[0]?.type?.startsWith('image/') && files[0].blob) {
      ...
      if (destatedIds.length > 0) cleanupOrphanedTemporaryFiles(destatedIds).catch(e => console.error('[ImageBlur New Selection] Cleanup failed:', e));

    } else if (files?.length) {
      setUiError(`Selected file "${files[0].filename}" is not a recognized image type.`);
    }
  }, [toolState.blurAmount, toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, clearProcessingHookOutput, cleanupOrphanedTemporaryFiles]);


  const handleBlurAmountChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const newBlurAmount = parseInt(e.target.value, 10);
    if (isNaN(newBlurAmount) || newBlurAmount < 0) return;

    const oldProcessedId = toolState.processedFileId;

    setState((prev) => ({ ...prev, blurAmount: newBlurAmount, processedFileId: null, lastUserGivenFilename: null }));
    await saveStateNow({ ...toolState, blurAmount: newBlurAmount, processedFileId: null, lastUserGivenFilename: null });
    setManualSaveSuccess(false);
    setDownloadAttempted(false);

    if (oldProcessedId) {
      cleanupOrphanedTemporaryFiles([oldProcessedId]).catch(e => console.error('[ImageBlur BlurChange] Cleanup call failed:', e));
    }
  ...
  const handleClear = useCallback(async () => {
    ...
    const currentBlurAmount = toolState.blurAmount;
    const currentAutoSave = toolState.autoSaveProcessed;
    const clearedState: ImageBlurToolState = {
      ...DEFAULT_BLUR_TOOL_STATE,
    ...
    if (destatedIds.length > 0) cleanupOrphanedTemporaryFiles(destatedIds).catch(err => console.error(`[ImageBlur Clear] Cleanup call failed:`, err));
  }, [toolState.blurAmount, toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  if (isLoadingToolSettings && !initialToolStateLoadCompleteRef.current) {
    return <p className="text-center p-4 italic text-gray-500 animate-pulse">Loading Image Blur Tool...</p>;
  }
    ...
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      <div className="flex flex-col gap-3 p-3 rounded-md bg-[rgb(var(--color-bg-subtle))] border border-[rgb(var(--color-border-base))]">
        ...
        <div className="flex flex-wrap gap-3 items-center pt-3 border-t border-gray-200 mt-2">
          <Checkbox
            label="Auto-save blurred image to Library"
            checked={toolState.autoSaveProcessed}
            onChange={handleAutoSaveChange}
            disabled={isProcessingImage || isManuallySaving}
            id="autoSaveBlurredImage"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          <label className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">
            Blurred Image {processedOutputPermanent && processedStoredFileForItde?.filename && <span className="font-normal text-xs">({processedStoredFileForItde.filename})</span>}
          </label>
          <div className="w-full aspect-square border rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
      ...
      <FilenamePromptModal
        ...
        title={filenamePromptAction === 'save' ? 'Save Blurred Image to Library' : 'Download Blurred Image'}
