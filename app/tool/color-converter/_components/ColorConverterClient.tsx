...
export default function ColorConverterClient({
    ...
    const handleConvert = useCallback((triggerType: TriggerType, sourceFieldOverride?: InputMode) => {
        ...
        console.log(`[ColorConverter] handleConvert triggered. Source: ${source}, Trigger: ${triggerType}`);
        ...
        try {
            if (source === 'hex') {
                ...
                if (rgbResult) {
                    ...
                    outputDetails = { rgb: `rgb(${currentR}, ${currentG}, ${currentB})`, hsl: `hsl(${currentH}, ${currentS}%, ${currentL}%)`, hex: currentHex }; // Include hex in output too
                } else { throw new Error('Invalid Hex code format. Use #RRGGBB or RRGGBB.'); }
            } else if (source === 'rgb') {
                ...
                if (isNaN(currentR) || isNaN(currentG) || isNaN(currentB) || currentR < 0 || currentR > 255 || currentG < 0 || currentG > 255 || currentB < 0 || currentB > 255) { throw new Error('Invalid RGB value. Each component must be between 0 and 255.'); }
                currentHex = rgbToHex(currentR, currentG, currentB);
                const hslResult = rgbToHsl(currentR, currentG, currentB);
                ({ h: currentH, s: currentS, l: currentL } = hslResult);
                ...
                if (!h.trim() || !s.trim() || !l.trim()) throw new Error("HSL inputs cannot be empty.");
                 ...
                 if (isNaN(currentH) || isNaN(currentS) || isNaN(currentL) || currentH < 0 || currentH > 360 || currentS < 0 || currentS > 100 || currentL < 0 || currentL > 100) { throw new Error('Invalid HSL value. H: 0-360, S/L: 0-100.'); }
                const rgbResult = hslToRgb(currentH, currentS, currentL);
                ({ r: currentR, g: currentG, b: currentB } = rgbResult);
                currentHex = rgbToHex(currentR, currentG, currentB);
                outputDetails = { hex: currentHex, rgb: `rgb(${currentR}, ${currentG}, ${currentB})` };
            } else { throw new Error("Invalid conversion source specified."); }
    ...
    }, [hex, r, g, b, h, s, l, lastEditedField, addHistoryEntry, toolTitle, toolRoute]); // Dependencies remain the same
    ...
    useEffect(() => {
        if (shouldRunOnLoad) {
            ...
            if (hex && hex.trim()) { sourceToUse = 'hex'; }
            else if (r && r.trim() && g && g.trim() && b && b.trim()) { sourceToUse = 'rgb'; }
            else if (h && h.trim() && s && s.trim() && l && l.trim()) { sourceToUse = 'hsl'; }
    ...
    const handleHexChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        setHex(event.target.value); setLastEditedField('hex'); setError(''); setCopiedFormat(null);
    }, []);
    const handleRgbChange = useCallback((event: React.ChangeEvent<HTMLInputElement>, component: 'r' | 'g' | 'b') => {
        const value = event.target.value; const setter = component === 'r' ? setR : component === 'g' ? setG : setB;
        setter(value); setLastEditedField('rgb'); setError(''); setCopiedFormat(null);
    }, []);
    const handleHslChange = useCallback((event: React.ChangeEvent<HTMLInputElement>, component: 'h' | 's' | 'l') => {
        const value = event.target.value; const setter = component === 'h' ? setH : component === 's' ? setS : setL;
        setter(value); setLastEditedField('hsl'); setError(''); setCopiedFormat(null);
    }, []);
    ...
    const handleCopy = useCallback(async (format: CopiedFormat) => {
        if (!format) return; let textToCopy = ''; setError(''); setCopiedFormat(null);
        try {
            if (format === 'hex') { textToCopy = hex; }
            else if (format === 'rgb') { textToCopy = `rgb(${r}, ${g}, ${b})`; }
            else if (format === 'hsl') { textToCopy = `hsl(${h}, ${s}%, ${l}%)`; }
            if (!textToCopy || (format === 'hex' && !hex.trim()) || (format === 'rgb' && (!r.trim() || !g.trim() || !b.trim())) || (format === 'hsl' && (!h.trim() || !s.trim() || !l.trim()))) { throw new Error("No valid color value to copy."); }
            await navigator.clipboard.writeText(textToCopy); setCopiedFormat(format); setTimeout(() => setCopiedFormat(null), 2000);
        } catch (err) { const message = err instanceof Error ? err.message : "Failed to copy."; setError(`Copy Error: ${message}`); console.error(`Copy ${format} failed:`, err); }
    }, [hex, r, g, b, h, s, l]);


    // --- Color Swatch Style (remains the same) ---
    const colorSwatchStyle = useMemo(() => {
        let backgroundColor = 'transparent'; let borderColor = 'rgb(var(--color-input-border))';
        if (!error) {
            const rNum = parseInt(r, 10); const gNum = parseInt(g, 10); const bNum = parseInt(b, 10);
            if (!isNaN(rNum) && !isNaN(gNum) && !isNaN(bNum) && rNum >= 0 && rNum <= 255 && gNum >= 0 && gNum <= 255 && bNum >= 0 && bNum <= 255) { backgroundColor = `rgb(${rNum}, ${gNum}, ${bNum})`; }
        }
         if (error) { borderColor = 'rgb(var(--color-border-error))'; }
        ...
        <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-5">
                ...
                <div className="md:col-span-1 space-y-1">
                    ...
                    <div className="flex">
                        <input type="text" id="hex-input" value={hex} onChange={handleHexChange} placeholder="#ffffff" className={`flex-grow p-2 border rounded-l-md shadow-sm focus:outline-none text-base font-mono ${error && lastEditedField === 'hex' ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-[rgb(var(--color-input-border))] focus:border-[rgb(var(--color-input-focus-border))] focus:ring-indigo-500'} bg-[rgb(var(--color-input-bg))] text-[rgb(var(--color-input-text))] placeholder:text-[rgb(var(--color-input-placeholder))]`} aria-invalid={error && lastEditedField === 'hex' ? 'true' : 'false'} aria-describedby={error && lastEditedField === 'hex' ? "input-error-feedback" : undefined}/>
                         <button type="button" onClick={() => handleCopy('hex')} title="Copy Hex" className={`px-3 border-y border-r rounded-r-md transition-colors ${copiedFormat === 'hex' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!hex || error ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!hex || !!error}>{copiedFormat === 'hex' ? 'Copied!' : 'Copy'}</button>
                    </div>
                </div>
                {/* RGB Inputs */}
                 <div className="md:col-span-1 space-y-1">
                    <label className={`block text-sm font-medium mb-1 ${lastEditedField === 'rgb' ? 'text-[rgb(var(--color-text-base))] font-semibold' : 'text-[rgb(var(--color-text-muted))]'}`}>RGB (0-255)</label>
                     <div className="flex items-stretch gap-1">
                         {([ { comp: 'r', value: r, setter: handleRgbChange, label: 'R' }, { comp: 'g', value: g, setter: handleRgbChange, label: 'G' }, { comp: 'b', value: b, setter: handleRgbChange, label: 'B' } ] as const).map(({ comp, value, setter, label }) => ( <input key={comp} type="number" min="0" max="255" value={value} placeholder={label} onChange={(e) => setter(e, comp)} aria-label={`RGB ${label}`} className={`w-1/3 p-2 border rounded-md shadow-sm focus:outline-none text-base ${error && lastEditedField === 'rgb' ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-[rgb(var(--color-input-border))] focus:border-[rgb(var(--color-input-focus-border))] focus:ring-indigo-500'} bg-[rgb(var(--color-input-bg))] text-[rgb(var(--color-input-text))] placeholder:text-[rgb(var(--color-input-placeholder))]`} aria-invalid={error && lastEditedField === 'rgb' ? 'true' : 'false'}/> ))}
                         <button type="button" onClick={() => handleCopy('rgb')} title="Copy RGB" className={`px-3 border rounded-md transition-colors ${copiedFormat === 'rgb' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!r || !g || !b || error ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!r || !g || !b || !!error}>{copiedFormat === 'rgb' ? 'Copied!' : 'Copy'}</button>
                     </div>
                 </div>
                {/* HSL Inputs */}
                 <div className="md:col-span-1 space-y-1">
                    ...
                    <div className="flex items-stretch gap-1">
                        {([ { comp: 'h', value: h, setter: handleHslChange, label: 'H', max: 360 }, { comp: 's', value: s, setter: handleHslChange, label: 'S', max: 100 }, { comp: 'l', value: l, setter: handleHslChange, label: 'L', max: 100 } ] as const).map(({ comp, value, setter, label, max }) => ( <input key={comp} type="number" min="0" max={max} value={value} placeholder={label} onChange={(e) => setter(e, comp)} aria-label={`HSL ${label}`} className={`w-1/3 p-2 border rounded-md shadow-sm focus:outline-none text-base ${error && lastEditedField === 'hsl' ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-[rgb(var(--color-input-border))] focus:border-[rgb(var(--color-input-focus-border))] focus:ring-indigo-500'} bg-[rgb(var(--color-input-bg))] text-[rgb(var(--color-input-text))] placeholder:text-[rgb(var(--color-input-placeholder))]`} aria-invalid={error && lastEditedField === 'hsl' ? 'true' : 'false'}/> ))}
                         <button type="button" onClick={() => handleCopy('hsl')} title="Copy HSL" className={`px-3 border rounded-md transition-colors ${copiedFormat === 'hsl' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!h || !s || !l || error ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!h || !s || !l || !!error}>{copiedFormat === 'hsl' ? 'Copied!' : 'Copy'}</button>
                    </div>
                 </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 items-center border-t border-b border-[rgb(var(--color-border-base))] py-4">
                 <button type="button" onClick={() => handleConvert('click', undefined)} className="px-6 py-2 rounded-md text-[rgb(var(--color-button-primary-text))] font-medium bg-[rgb(var(--color-button-primary-bg))] hover:bg-[rgb(var(--color-button-primary-hover-bg))] focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[rgb(var(--color-button-primary-bg))] transition-colors duration-150 ease-in-out disabled:bg-[rgb(var(--color-bg-disabled))] disabled:cursor-not-allowed disabled:text-[rgb(var(--color-text-muted))]" disabled={!hex.trim() && (!r.trim() || !g.trim() || !b.trim()) && (!h.trim() || !s.trim() || !l.trim())}> Convert </button>
