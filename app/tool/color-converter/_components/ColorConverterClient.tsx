...
export default function ColorConverterClient({
  ...
  const handleConvert = useCallback(
    (triggerType: TriggerType, sourceFieldOverride?: InputMode) => {
      ...
      console.log(
        `[ColorConverter] handleConvert triggered. Source: ${source}, Trigger: ${triggerType}`
      );
      ...
      try {
        if (source === 'hex') {
          ...
          if (!r.trim() || !g.trim() || !b.trim())
            throw new Error('RGB inputs cannot be empty.');
          currentR = parseInt(r, 10);
          currentG = parseInt(g, 10);
          ...
          if (!h.trim() || !s.trim() || !l.trim())
            throw new Error('HSL inputs cannot be empty.');
          currentH = parseInt(h, 10);
          currentS = parseInt(s, 10);
          currentL = parseInt(l, 10);
          inputDetails = {
            h: String(currentH),
            s: String(currentS),
            l: String(currentL),
        ...
        setHex(currentHex);
        setR(String(currentR));
        setG(String(currentG));
        setB(String(currentB));
        setH(String(currentH));
        setS(String(currentS));
        setL(String(currentL));
      } catch (err) {
        currentError =
        ...
        if (source === 'hex') inputDetails = { hex: hex, error: currentError };
          ...
          inputDetails = { r: r, g: g, b: b, error: currentError };
        else if (source === 'hsl')
          inputDetails = { h: h, s: s, l: l, error: currentError };
    ...
    async (format: CopiedFormat) => {
      ...
      try {
        ...
        if (
          !textToCopy ||
          (format === 'hex' && !hex.trim()) ||
          (format === 'rgb' && (!r.trim() || !g.trim() || !b.trim())) ||
          (format === 'hsl' && (!h.trim() || !s.trim() || !l.trim()))
        ) {
    ...
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-5">
        ...
        <div className="md:col-span-1 space-y-1">
          <label
            htmlFor="hex-input"
            className={`block text-sm font-medium mb-1 ${lastEditedField === 'hex' ? 'text-[rgb(var(--color-text-base))] font-semibold' : 'text-[rgb(var(--color-text-muted))]'}`}
          >
            Hex Color (#RRGGBB)
          </label>
          <div className="flex">
            <input
              ...
              aria-invalid={
                error && lastEditedField === 'hex' ? 'true' : 'false'
              }
              aria-describedby={
                error && lastEditedField === 'hex'
            ...
            <button
              ...
              title="Copy Hex"
              className={`px-3 border-y border-r rounded-r-md transition-colors ${copiedFormat === 'hex' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!hex || error ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={!hex || !!error}
            >
        ...
        <div className="md:col-span-1 space-y-1">
          <label
            className={`block text-sm font-medium mb-1 ${lastEditedField === 'rgb' ? 'text-[rgb(var(--color-text-base))] font-semibold' : 'text-[rgb(var(--color-text-muted))]'}`}
          >
            RGB (0-255)
          </label>
          <div className="flex items-stretch gap-1">
            {(
              ...
              <input
                ...
                aria-invalid={
                  error && lastEditedField === 'rgb' ? 'true' : 'false'
                }
              />
            ))}
            <button
              ...
              title="Copy RGB"
              className={`px-3 border rounded-md transition-colors ${copiedFormat === 'rgb' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!r || !g || !b || error ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={!r || !g || !b || !!error}
            >
        ...
        <div className="md:col-span-1 space-y-1">
          <label
            className={`block text-sm font-medium mb-1 ${lastEditedField === 'hsl' ? 'text-[rgb(var(--color-text-base))] font-semibold' : 'text-[rgb(var(--color-text-muted))]'}`}
          >
          ...
          <div className="flex items-stretch gap-1">
            ...
            ).map(({ comp, value, setter, label, max }) => (
              <input
                key={comp}
                type="number"
                min="0"
                ...
                aria-invalid={
                  error && lastEditedField === 'hsl' ? 'true' : 'false'
                }
              />
            ))}
            <button
              ...
              title="Copy HSL"
              className={`px-3 border rounded-md transition-colors ${copiedFormat === 'hsl' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-[rgb(var(--color-button-neutral-bg))] text-[rgb(var(--color-button-neutral-text))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))]'} ${!h || !s || !l || error ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={!h || !s || !l || !!error}
            >
      ...
      <div className="flex flex-wrap gap-4 items-center border-t border-b border-[rgb(var(--color-border-base))] py-4">
        <button
          ...
          disabled={
            !hex.trim() &&
            (!r.trim() || !g.trim() || !b.trim()) &&
            (!h.trim() || !s.trim() || !l.trim())
          }
        >
          {' '}
          Convert{' '}
        ...
        <button
          type="button"
          onClick={handleClear}
          className="px-4 py-2 rounded-md text-[rgb(var(--color-button-neutral-text))] font-medium bg-[rgb(var(--color-button-neutral-bg))] hover:bg-[rgb(var(--color-button-neutral-hover-bg))] focus:outline-none transition duration-150 ease-in-out"
        >
          Clear
        </button>
        <div className="ml-auto flex items-center gap-2">
          ...
          <div
            className="h-8 w-14 rounded shadow-inner"
