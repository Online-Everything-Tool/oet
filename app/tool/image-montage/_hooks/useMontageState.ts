...
export function useMontageState(toolTitle: string, toolRoute: string): UseMontageStateReturn {
    ...
    const addImagesFromFiles = useCallback(async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
        ...
        try {
            ...
            if (newImages.length === 0 && addedFileNames.length > 0) { throw new Error("No valid image files processed."); }
    ...
    const handleTiltChange = useCallback((imageId: number, newTilt: number) => { setMontageImages((prevImages) => prevImages.map((img) => img.id === imageId ? { ...img, tilt: newTilt } : img)); }, []);
    const handleOverlapChange = useCallback((imageId: number, newOverlap: number) => { setMontageImages((prevImages) => prevImages.map((img) => img.id === imageId ? { ...img, overlapPercent: Math.max(0, Math.min(MAX_OVERLAP_PERCENT, newOverlap)) } : img)); }, []);
    const handleMoveImageLeft = useCallback((indexToMove: number) => { if (indexToMove <= 0) return; setMontageImages(prevImages => { const newImages = [...prevImages]; [newImages[indexToMove - 1], newImages[indexToMove]] = [newImages[indexToMove], newImages[indexToMove - 1]]; return newImages; }); }, []);
    const handleMoveImageRight = useCallback((indexToMove: number) => { setMontageImages(prevImages => { if (indexToMove >= prevImages.length - 1) return prevImages; const newImages = [...prevImages]; [newImages[indexToMove + 1], newImages[indexToMove]] = [newImages[indexToMove], newImages[indexToMove + 1]]; return newImages; }); }, []);

    return {
        montageImages, addImagesFromFiles, clearMontage, handleTiltChange, handleOverlapChange,
        handleMoveImageLeft, handleMoveImageRight, isLoading, error, setIsLoading, setError,
    };
}