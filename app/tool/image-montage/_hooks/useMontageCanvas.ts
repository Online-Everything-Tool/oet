...
const calculateMaxBoundsLocal = (width: number, height: number): { maxW: number; maxH: number } => {
    const diagonal = Math.sqrt(width * width + height * height);
    return { maxW: diagonal, maxH: diagonal };
};

const calculateRenderedBoundsLocal = (images: MontageImage[], mainCanvasHeight: number): RenderedBounds | null => {
    if (images.length === 0) return null;
    const { maxW, maxH } = calculateMaxBoundsLocal(TOTAL_POLAROID_WIDTH, TOTAL_POLAROID_HEIGHT);
...
export function useMontageCanvas(montageImages: MontageImage[]): UseMontageCanvasReturn {
    ...
    useEffect(() => {
        ...
        const { maxH } = calculateMaxBoundsLocal(TOTAL_POLAROID_WIDTH, TOTAL_POLAROID_HEIGHT);
        ...
        montageImages.forEach((imgData, index) => {
            ...
            ctx.shadowColor = 'rgba(0, 0, 0, 0.3)'; ctx.shadowBlur = 10; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 3;
            ctx.fillStyle = `rgb(${componentBgColor})`; ctx.fillRect(polaroidX, polaroidY, TOTAL_POLAROID_WIDTH, TOTAL_POLAROID_HEIGHT);
            ...
            ctx.strokeStyle = `rgba(${borderBaseColor}, 0.3)`; ctx.lineWidth = 0.5;
    ...
    const generateMontageBlob = useCallback(async (): Promise<Blob | null> => {
        ...
        try {
            const bounds = calculateRenderedBoundsLocal(montageImages, mainCanvas.height);
            ...
            tempCtx.drawImage(
                mainCanvas,
                bounds.minX, bounds.minY, bounds.width, bounds.height, // Source rect
                0, 0, bounds.width, bounds.height // Destination rect
            );
            const blob = await new Promise<Blob | null>((resolve) => tempCanvas.toBlob(resolve, 'image/png', 0.95));
            if (!blob) {
