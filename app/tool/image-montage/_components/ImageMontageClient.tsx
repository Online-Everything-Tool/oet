  ...
  const handleSaveToLibrary = useCallback(async () => {
    ...
    if (blob) {
      ...
      try {
        ...
        historyOutput = {
          message: `Saved montage (${montageImages.length} images) to library.`,
          imageId: newImageId,
        };
        setIsSaved(true);
  ...
  const handleDownload = useCallback(async () => {
    ...
    if (blob) {
      ...
      try {
        ...
        console.error(
          '[MontageDownload] Failed to save montage to library:',
          saveErrorMsg
        );
      }
      ...
      if (newImageId) {
        historyOutput = {
          message: `Downloaded and saved montage (${montageImages.length} images).`,
          imageId: newImageId,
        };
      } else {
        historyOutput = {
          message: `Downloaded montage (${montageImages.length} images). Save to library failed.`,
          error: saveErrorMsg,
        };
      }
    } else {
    ...
    if (blob) {
      ...
      try {
        ...
        saveErrorMsg =
          saveErr instanceof Error ? saveErr.message : String(saveErr);
        console.error(
          '[MontageCopy] Failed to save montage to library:',
          saveErrorMsg
        );
      }
      ...
      try {
        ...
        if (newImageId) {
          historyOutput = {
            message: `Copied and saved montage (${montageImages.length} images).`,
            imageId: newImageId,
          };
        } else {
          historyOutput = {
            message: `Copied montage (${montageImages.length} images). Save to library failed.`,
            error: saveErrorMsg,
          };
        }
