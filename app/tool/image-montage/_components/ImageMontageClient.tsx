...
export default function ImageMontageClient({ toolTitle, toolRoute }: ImageMontageClientProps) {
    ...
    // Define clearError within the component scope
    const clearError = useCallback(() => {
        setActionError(null);
        setFileProcessingError(null);
    }, [setFileProcessingError]); // Dependency added
    ...
    const handleSaveToLibrary = useCallback(async () => {
        ...
        addHistoryEntry({
            ...
            output: historyOutput, status: status,
        });
    }, [generateMontageBlob, addImage, montageImages.length, addHistoryEntry, toolTitle, toolRoute, clearError]); // Added clearError dependency (line 92 related)
    ...
    const handleDownload = useCallback(async () => {
        ...
        if (blob) {
            ...
            URL.revokeObjectURL(url);
             ...
             if (newImageId) {
                historyOutput = { message: `Downloaded and saved montage (${montageImages.length} images).`, imageId: newImageId };
             } else {
                historyOutput = { message: `Downloaded montage (${montageImages.length} images). Save to library failed.`, error: saveErrorMsg };
             }
        } else {
            status = 'error';
    ...
    }, [generateMontageBlob, addImage, montageImages.length, addHistoryEntry, toolTitle, toolRoute, clearError]); // Added clearError dependency (line 142 related)
        ...
        if (blob) {
            const outputFileName = `oet-montage-copy-${Date.now()}.png`;
             try {
                 ...
                 saveErrorMsg = saveErr instanceof Error ? saveErr.message : String(saveErr);
                 console.error("[MontageCopy] Failed to save montage to library:", saveErrorMsg);
             }
            ...
            try {
                ...
                setTimeout(() => setIsCopied(false), 2000);
                 ...
                 if (newImageId) {
                    historyOutput = { message: `Copied and saved montage (${montageImages.length} images).`, imageId: newImageId };
                 } else {
                    historyOutput = { message: `Copied montage (${montageImages.length} images). Save to library failed.`, error: saveErrorMsg };
                 }

            } catch (clipboardError) {
                status = 'error';
                const message = clipboardError instanceof Error ? clipboardError.message : 'Unknown clipboard write error';
                console.error('Failed to write image to clipboard:', clipboardError);
    ...
    const handleAdjustment = useCallback( (action: (...args: any[]) => void) => (...args: any[]) => {
        ...
        setIsCopied(false);
        action(...args);
    }, []); // Corrected type signature (line 214 related)
    ...
    return (
        <div className="flex flex-col gap-4 text-[rgb(var(--color-text-base))]">
            <MontageControls
                ...
                onClear={() => { clearMontage(); setIsSaved(false); setIsCopied(false); }}
                    ...
                    <div className="flex space-x-4 overflow-x-auto py-2 px-1">
                        {montageImages.map((img, index) => (
                            <ImageAdjustmentCard
                                ...
                                index={index}
                                isFirst={index === 0}
                                isLast={index === montageImages.length - 1}
                                isLoading={isLoading}
                                onTiltChange={handleAdjustment(handleTiltChange)}
                                onOverlapChange={handleAdjustment(handleOverlapChange)}
                                onMoveLeft={handleAdjustment(handleMoveImageLeft)}
                                onMoveRight={handleAdjustment(handleMoveImageRight)}
                            />
                        ))}
                    </div>
                </div>
