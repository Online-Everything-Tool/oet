...
export default function ImageAdjustmentCard({
    image, index, isFirst, isLast, isLoading,
    onTiltChange, onOverlapChange, onMoveLeft, onMoveRight
    ...
    return (
        <div className="flex-shrink-0 flex flex-col items-center space-y-2 p-3 border border-gray-200 rounded-lg bg-white shadow-sm w-[180px]">
            ...
            <div className="flex justify-between items-center w-full mb-1">
                ...
                <p className="text-xs font-medium text-gray-600 text-center flex-grow mx-1 truncate" title={image.alt}>
                    {index + 1}. {image.alt}
                </p>
                <button
                    type="button"
            ...
            <div className="w-full">
                <label htmlFor={`tilt-${image.id}`} className="text-[10px] text-gray-500 block text-center mb-0.5">Tilt ({image.tilt}°)</label>
                <input
                    id={`tilt-${image.id}`} type="range" min={-MAX_TILT_DEG} max={MAX_TILT_DEG} step="1" value={image.tilt}
            ...
            {!isFirst && ( // Show overlap only for images after the first
                <div className="w-full">
                    <label htmlFor={`overlap-${image.id}`} className="text-[10px] text-gray-500 block text-center mb-0.5">Overlap ({image.overlapPercent}%)</label>
                    <input
                        id={`overlap-${image.id}`} type="range" min="0" max={MAX_OVERLAP_PERCENT} step="1" value={image.overlapPercent}
