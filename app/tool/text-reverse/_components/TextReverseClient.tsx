...
const TextReverseClient = ({ urlStateParams, toolTitle, toolRoute }: TextReverseClientProps) => {
    ...
    const handleReverseChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const handleTextBlur = useCallback(() => {
        ...
        if (textChanged || reverseChanged) {
            const limitedOutput = reversedText.length > 500 ? reversedText.substring(0, 500) + '...' : reversedText;
    ...
    return (
        <div className="flex flex-col gap-4 text-[rgb(var(--color-text-base))]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div>
                    <label htmlFor="text-input" className="block text-sm font-medium text-[rgb(var(--color-text-muted))]"> Input Text: </label>
                    <textarea
                        id="text-input"
                        rows={8}
                        value={text}
                 ...
                 <div>
                    <label htmlFor="reversed-text" className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">Reversed Text:</label>
                    <div
                        id="reversed-text"
                        ...
                        aria-live="polite"
                    >
                        {reversedText || <span className="italic text-[rgb(var(--color-input-placeholder))]">Reversed text will appear here...</span>}
                    </div>
                 </div>
            </div>
            <div className="flex flex-wrap items-center gap-4 border border-[rgb(var(--color-border-base))] rounded-md p-4 bg-[rgb(var(--color-bg-component))]">
                <label htmlFor="reverse-select" className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">Reverse by:</label>
                <select
                    id="reverse-select"
                    name="reverse"
                    value={reverse}
