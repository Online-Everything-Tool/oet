    ...
    const handlePaste = useCallback(async (event: React.ClipboardEvent<HTMLDivElement>) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (item.kind === 'file') {
                ...
                if (blob) {
                    foundFile = true;
                    const file = new File([blob], blob.name || `pasted-file-${Date.now()}`, { type: blob.type });
                    fileList.push(file);
                } else { console.warn(`[FileDropZone] item.getAsFile() returned null for item type ${item.type}.`); }
            }
        }
    ...
    return (
        ...
        >
            ...
            {isDragging && (
             ...
             {isPasting && !isLoading && (
                 <div className="absolute inset-0 bg-blue-100 bg-opacity-50 flex items-center justify-center pointer-events-none z-10">
                     <p className="text-blue-700 font-semibold text-lg animate-pulse">Processing Paste...</p>
                 </div>
              )}

            {/* Render the actual content (list/grid) passed as children */}
            {children}
