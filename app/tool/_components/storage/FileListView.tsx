...
export default function FileListView({
    ...
    const handleRowClick = (e: React.MouseEvent<HTMLTableRowElement>, fileId: string) => {
        const target = e.target as HTMLElement;
        if (target.closest('[data-cell="actions"]') || target.tagName === 'BUTTON' || target.tagName === 'A') {
             return;
        }
         if (target.closest('[data-cell="checkbox"]') || target.tagName === 'INPUT') {
             return;
         }
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Size</th>
                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Added</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {files.map((file) => {
                 ...
                 return (
                    ...
                    >
                        {/* Checkbox Cell */}
                         <td className="pl-4 pr-2 py-2 whitespace-nowrap" data-cell="checkbox">
                             <input
                                type="checkbox"
                                checked={isSelected}
                        ...
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500 truncate max-w-xs" title={file.type}>{file.type || 'N/A'}</td>
                        ...
                        {/* Actions Cell */}
                        <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-medium" data-cell="actions">
                            <div className="flex items-center justify-end gap-1.5">
                                  {(file.type === 'application/zip' || file.type === 'application/x-zip-compressed') && (
                                      <button onClick={(e) => { handleActionClick(e); onSendTo(file.id); }} disabled={isProcessing} title="Send to ZIP Explorer" className="p-1 text-blue-600 rounded hover:bg-blue-100 focus:outline-none focus:ring-1 focus:ring-blue-400 disabled:opacity-50">➡️</button>
                                  )}
                                 <button onClick={(e) => { handleActionClick(e); onCopy(file.id); }} disabled={!isTextFile || isProcessing || currentFeedback?.type === 'copy'} title={isTextFile ? "Copy file content" : "Cannot copy content"} className={`p-1 rounded focus:outline-none focus:ring-1 disabled:opacity-50 disabled:cursor-not-allowed ${currentFeedback?.type === 'copy' ? 'bg-green-100 text-green-700 ring-1 ring-green-300' : 'text-green-600 hover:bg-green-100 focus:ring-green-400'}`}>{currentFeedback?.type === 'copy' ? '✔️' : '📄'}</button>
                                 <button onClick={(e) => { handleActionClick(e); onDownload(file.id); }} disabled={isProcessing || currentFeedback?.type === 'download'} title="Download this file" className={`p-1 rounded focus:outline-none focus:ring-1 disabled:opacity-50 ${currentFeedback?.type === 'download' ? 'bg-indigo-100 text-indigo-700 ring-1 ring-indigo-300' : 'text-indigo-600 hover:bg-indigo-100 focus:ring-indigo-400'}`}>{currentFeedback?.type === 'download' ? '✔️' : '⬇️'}</button>
                                 <button onClick={(e) => { handleActionClick(e); onDelete(file.id); }} disabled={isProcessing || isSelected} title={isSelected ? "Use 'Delete Selected' button" : "Delete this file"} className="p-1 text-red-600 rounded hover:bg-red-100 focus:outline-none focus:ring-1 focus:ring-red-400 disabled:opacity-50 disabled:cursor-not-allowed">❌</button>
                            </div>
                        </td>
                         {/* Error Feedback Overlay */}
                         {currentFeedback?.type === 'error' && (
                             <tr className="absolute inset-0 pointer-events-none" aria-hidden="true">
                                <td colSpan={6}>
                                     <div className="absolute inset-x-0 bottom-0 p-1 bg-red-100 text-red-700 text-[10px] text-center truncate border-t border-red-200" title={currentFeedback.message}>
             ...
             {isLoading && !isBulkDeleting && ( // Show general loading only if not bulk deleting
                <p className="text-center text-gray-500 italic py-8 animate-pulse">Loading...</p>
             )}
