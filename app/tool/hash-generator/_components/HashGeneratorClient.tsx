    ...
    async (triggerType: TriggerType, textToProcess = text) => {
      ...
      try {
        if (algorithm === 'MD5') {
          ...
          if (!crypto?.subtle) {
            throw new Error(
              'Web Crypto API (crypto.subtle) is not available in this browser or context (requires HTTPS).'
            );
          }
        ...
        errorMessage =
          err instanceof Error
            ? err.message
            : 'An unexpected error occurred during hashing.';
  ...
  return (
    <div className="space-y-6 text-[rgb(var(--color-text-base))]">
      ...
      {error && (
        <div
          role="alert"
          className="p-3 border rounded-md text-sm bg-[rgb(var(--color-bg-error-subtle))] border-[rgb(var(--color-border-error))] text-[rgb(var(--color-text-error))]"
        >
          {' '}
      ...
      {(outputValue || isLoading) && (
        <div>
          ...
          <textarea
            ...
            className={`w-full p-3 border border-[rgb(var(--color-input-border))] bg-[rgb(var(--color-bg-subtle))] text-[rgb(var(--color-input-text))] rounded-md shadow-sm resize-none text-base font-mono focus:border-[rgb(var(--color-input-focus-border))] focus:outline-none placeholder:text-[rgb(var(--color-input-placeholder))] ${isLoading ? 'animate-pulse' : ''}`}
            aria-live="polite"
