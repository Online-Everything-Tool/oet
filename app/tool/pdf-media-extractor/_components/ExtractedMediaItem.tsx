...
import { ArrowDownTrayIcon, DocumentPlusIcon, TrashIcon, CheckIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="relative group border rounded-md shadow-sm overflow-hidden bg-white flex flex-col transition-all duration-150 ease-in-out">
      ...
      <div className="p-3 border-t border-[rgb(var(--color-border-base))] flex-grow flex flex-col">
        <p className="text-xs font-medium text-[rgb(var(--color-text-emphasis))] truncate" title={file.filename}>
          {file.filename}
        </p>
      ...
      <div className="p-2 bg-[rgb(var(--color-bg-subtle))] border-t border-[rgb(var(--color-border-base))] flex justify-end items-center gap-1.5">
        <Button variant="danger-outline" size="sm" onClick={() => onDelete(file.id)} title="Delete">
          <TrashIcon className="h-4 w-4" />
        </Button>
        <Button variant="primary-outline" size="sm" onClick={() => onSave(file)} title="Save to Library" disabled={!file.isTemporary}>
          {file.isTemporary ? <DocumentPlusIcon className="h-4 w-4" /> : <CheckIcon className="h-4 w-4" />}
        </Button>
        <Button variant="secondary-outline" size="sm" onClick={() => onDownload(file)} title="Download">
