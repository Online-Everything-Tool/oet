...
export default function PdfMediaExtractorClient({
  ...
  const {
    ...
    saveStateNow,
  } = useToolState<PdfExtractorState>(toolRoute, DEFAULT_STATE);
  const {
    getFile,
  ...
  useEffect(() => {
    if (state.inputPdfId && !isExtracting) {
      const hasBeenProcessed =
        state.outputMediaIds.length > 0 || extractionError;
      if (!hasBeenProcessed) {
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      ...
      const files = (
        await Promise.all(state.outputMediaIds.map((id) => getFile(id)))
  ...
  const handleSaveItem = async (file: StoredFile) => {
    ...
    setExtractedFiles((prev) =>
      prev.map((f) => (f.id === file.id ? { ...f, isTemporary: false } : f))
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 border border-[rgb(var(--color-border-base))] rounded-md bg-[rgb(var(--color-bg-subtle))] space-y-3">
          ...
          {state.selectedOutputIds.length > 0 && (
            <SendToToolButton
              ...
              currentToolOutputConfig={
                toolSpecificMetadata.outputConfig as OutputConfig
              }
        ...
        {state.inputPdfName && !isExtracting && (
          <p className="text-sm text-[rgb(var(--color-text-muted))]">
            Loaded: <strong>{state.inputPdfName}</strong>
          </p>
        )}
      </div>
      ...
      {!isExtracting && extractedFiles.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold mb-2">
            Extracted Media ({extractedFiles.length})
          </h3>
      ...
      {!isExtracting && !state.inputPdfId && !isLoadingState && (
        <div className="text-center p-8 border-2 border-dashed border-[rgb(var(--color-border-soft))] rounded-lg">
          <p className="text-lg text-[rgb(var(--color-text-muted))]">
            Select a PDF file to start extracting images.
          </p>
        </div>
      )}
