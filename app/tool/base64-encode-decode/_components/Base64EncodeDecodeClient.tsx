  ...
  const handleEncode = useCallback(
    (triggerType: TriggerType, textToProcess = text) => {
      ...
      try {
        ...
        currentError =
          'Failed to encode text to Base64. Ensure text is valid UTF-8.';
    ...
    (triggerType: TriggerType, textToProcess = text) => {
      ...
      try {
        ...
        if (
          ...
          currentError =
            'Failed to decode: Input contains characters that are not valid Base64.';
        } else {
  ...
  const getLikelihoodBarState = () => {
    switch (base64Likelihood) {
      ...
      case 'possibly_base64_or_text':
        return {
          text: 'Format: Potentially Base64',
  ...
  return (
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      <div>
        ...
        />
        <div
          className="relative h-3 mt-1 bg-[rgb(var(--color-indicator-track-bg))] rounded-full overflow-hidden"
          title={`Input Format Likelihood: ${label}`}
        >
          <div
            className={`absolute inset-0 ${bgColor} rounded-full transition-all duration-300 ease-in-out w-full`}
            role="progressbar"
            aria-label={`Input Format Likelihood: ${label}`}
        ...
        <p
          className="text-xs text-[rgb(var(--color-text-muted))] mt-1 h-4"
          id="format-indicator"
          aria-live="polite"
        >
          {likelihoodText}
