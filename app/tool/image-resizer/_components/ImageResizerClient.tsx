...
export default function ImageResizerClient({ toolRoute }: ImageResizerClientProps) {
  ...
  const resizeDrawFunction = useCallback(
    (ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
      ...
      ctx.clearRect(0, 0, toolState.targetWidth, toolState.targetHeight);
      const aspectRatio = w / h;
      let drawWidth = toolState.targetWidth;
      let drawHeight = toolState.targetHeight;

      if (aspectRatio > toolState.targetWidth / toolState.targetHeight) {
        drawHeight = toolState.targetWidth / aspectRatio;
      } else {
        drawWidth = toolState.targetHeight * aspectRatio;
      }

      const xOffset = (toolState.targetWidth - drawWidth) / 2;
      const yOffset = (toolState.targetHeight - drawHeight) / 2;

      ctx.drawImage(img, xOffset, yOffset, drawWidth, drawHeight);
    },
    [toolState.targetWidth, toolState.targetHeight]
  );
          ...
          <div className="flex gap-4">
            <Input
              ...
              onChange={(e) =>
                setState((prev) => ({
                  ...prev,
                  targetWidth: parseInt(e.target.value, 10) || 0,
                  processedFileId: null,
                }))
              }
              min={0}
            ...
            <Input
              ...
              onChange={(e) =>
                setState((prev) => ({
                  ...prev,
                  targetHeight: parseInt(e.target.value, 10) || 0,
                  processedFileId: null,
                }))
              }
              min={0}
