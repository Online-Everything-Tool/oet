...
export default function AspectRatioCalculatorClient({
  ...
  const { categorizedRatioConfigs, calculateOutputDimensions } =
    useAspectRatioDefinitions();
  ...
  useEffect(() => {
    if (!isLoadingUrlState && urlProvidedAnyValue && !isLoadingToolSettings) {
      ...
      if (
        widthFromUrl !== toolState.widthInput ||
        heightFromUrl !== toolState.heightInput
      ) {
        if (
          (widthFromUrl && widthFromUrl !== toolState.widthInput) ||
          (heightFromUrl && heightFromUrl !== toolState.heightInput)
        ) {
    ...
    setToolState,
    toolState.widthInput,
    toolState.heightInput,
  ]);
  ...
  const debouncedParseAndValidateInputs = useDebouncedCallback(() => {
    ...
    if (
      toolState.widthInput.trim() === '' &&
      toolState.heightInput.trim() === ''
    ) {
      currentError = '';
    }
  ...
  if (isLoadingToolSettings && !isLoadingUrlState) {
    return (
      <p className="text-center p-4">Loading Aspect Ratio Calculator...</p>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="p-4 border border-[rgb(var(--color-border-base))] rounded-lg bg-[rgb(var(--color-bg-subtle))] shadow-sm">
        ...
        {inputError && (
          <p className="mt-2 text-sm text-[rgb(var(--color-text-error))]">
            {inputError}
          </p>
        )}
      </div>
      ...
      {!parsedWidth && !parsedHeight && !inputError && (
        <p className="text-center text-gray-500 italic">
          Enter a width and/or height to see calculations.
        </p>
      )}
