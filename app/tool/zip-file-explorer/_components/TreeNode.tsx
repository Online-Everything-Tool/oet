...
const TreeNode: React.FC<TreeNodeProps> = ({
    ...
    const icon = node.type === 'folder'
        ? (isExpandable ? (isExpanded ? '▼' : '►') : '📁') // Use folder icon if not expandable
        : '📄'; // File icon
    ...
    return (
        <div>
            ...
            >
                ...
                {/* Name */}
                <span className="flex-grow truncate text-[rgb(var(--color-text-base))]" title={node.path}>
                    {node.name}
                </span>
                {/* Action Buttons */}
                {actionData && (
            ...
            {isExpanded && node.children && node.children.length > 0 && (
                <div className="border-l border-[rgba(var(--color-border-base)/0.5)] ml-[12px] pl-1"> {/* Added pl-1 for spacing */}
                    {node.children.sort((a, b) => {
                         if (a.type !== b.type) return a.type === 'folder' ? -1 : 1;
                         return a.name.localeCompare(b.name);
                     }).map(childNode => (
                        <TreeNode
                            key={childNode.id} node={childNode} level={level + 1}
                            expandedFolders={expandedFolders} onToggle={onToggle}
                            onDownload={onDownload} onPreview={onPreview}
                        />
                    ))}
                </div>
            )}
            ...
            {isExpanded && node.type === 'folder' && (!node.children || node.children.length === 0) && (
                 <div style={{ paddingLeft: `${(level + 1) * 1.5}rem` }} className="text-xs text-[rgb(var(--color-text-muted))] italic p-1">
                     (empty)
                 </div>
             )}
        </div>
