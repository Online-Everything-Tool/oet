...
export default function ZipFileExplorerClient({
  ...
  const handlePreview = useCallback(async (entryData: ActionEntryData) => {
    if (!entryData?._zipObject) {
      setPreviewError(
        `Preview error: Zip object missing for ${entryData.name}`
      );
    ...
    setIsPreviewOpen(true);
    setPreviewFilename(entryData.id);
  ...
  return (
    <div className="flex flex-col gap-4 text-[rgb(var(--color-text-base))]">
      ...
      {isPreviewOpen && (
        ...
        >
          <div
            className="bg-[rgb(var(--color-bg-component))] rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col text-[rgb(var(--color-text-base))]"
              ...
              {!previewError && previewType === 'image' && previewContent && (
                <div className="flex justify-center items-center h-full">
                  <Image
                    ...
                    onError={() =>
                      setPreviewError('Failed to load image resource.')
                    }
                    unoptimized={true}
                  />
                </div>
