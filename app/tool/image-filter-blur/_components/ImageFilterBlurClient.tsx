...
interface ImageFilterBlurToolState {
  blurRadius: number;
...
const DEFAULT_BLUR_TOOL_STATE: ImageFilterBlurToolState = {
  blurRadius: 5,
...
interface ImageFilterBlurClientProps {
  toolRoute: string;
}

export default function ImageFilterBlurClient({ toolRoute }: ImageFilterBlurClientProps) {
  const { state: toolState, setState, saveStateNow, isLoadingState: isLoadingToolSettings } = useToolState<ImageFilterBlurToolState>(toolRoute, DEFAULT_BLUR_TOOL_STATE);
  ...
  const { isLoading: isProcessingImage, error: processingErrorHook, processImage, clearProcessingOutput: clearProcessingHookOutput } = useImageProcessing();
  ...
  const generateDefaultOutputFilename = useCallback(() => {
    const originalName = originalFilenameForDisplay || 'blurred-image';
    ...
    return `blur-${baseName}.${extension}`;
  ...
  const handleProcessIncomingSignal = useCallback(async (signal: IncomingSignal) => {
    ...
    if (newSelectedFileId) {
      ...
      const newState: ImageFilterBlurToolState = {
        selectedFileId: newSelectedFileId,
        processedFileId: null,
        blurRadius: toolState.blurRadius,
      ...
      if (destatedIds.length > 0) {
        cleanupOrphanedTemporaryFiles(destatedIds).catch((e) => console.error('[ImageFilterBlur ITDE Accept] Cleanup call failed:', e));
      }
    }
  }, [getToolMetadata, toolState.blurRadius, toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  const applyBlurFilter = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
    const { naturalWidth: w, naturalHeight: h } = img;
    ctx.drawImage(img, 0, 0, w, h);
    ctx.filter = `blur(${toolState.blurRadius}px)`;
    ctx.drawImage(img, 0, 0, w, h);
  }, [toolState.blurRadius]);
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(inputFile, applyBlurFilter, outputFileName, {}, toolState.autoSaveProcessed);
      if (result.id) {
    ...
    triggerProcessing();
  }, [toolState.selectedFileId, toolState.processedFileId, toolState.autoSaveProcessed, isLoadingToolSettings, isProcessingImage, processImage, applyBlurFilter, getFile, setState, processingErrorHook]);
  ...
  const handleFilesSelectedFromModal = useCallback(async (files: StoredFile[]) => {
    ...
    if (files?.[0]?.type?.startsWith('image/') && files[0].blob) {
      ...
      if (destatedIds.length > 0) cleanupOrphanedTemporaryFiles(destatedIds).catch((e) => console.error('[ImageFilterBlur New Selection] Cleanup failed:', e));
    } else if (files?.length) {
      setUiError(`Selected file "${files[0].filename}" is not a recognized image type.`);
    }
  }, [toolState.blurRadius, toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, setState, saveStateNow, clearProcessingHookOutput, cleanupOrphanedTemporaryFiles]);

  const handleBlurRadiusChange = useCallback(async (newBlur: number) => {
    const newState = { ...toolState, blurRadius: newBlur, processedFileId: null, lastUserGivenFilename: null };
    setState(newState);
    await saveStateNow(newState);
    clearProcessingHookOutput();
  }, [toolState, setState, saveStateNow, clearProcessingHookOutput]);
  ...
  }, [toolState, processedOutputPermanent, isProcessingImage, isManuallySaving, makeFilePermanentAndUpdate, setState, saveStateNow]);

  const handleClear = useCallback(async () => {
    const oldSelectedId = toolState.selectedFileId;
    const oldProcessedId = toolState.processedFileId;
    const currentAutoSave = toolState.autoSaveProcessed;
    const currentBlur = toolState.blurRadius;
    const clearedState: ImageFilterBlurToolState = { ...DEFAULT_BLUR_TOOL_STATE, autoSaveProcessed: currentAutoSave, blurRadius: currentBlur, lastUserGivenFilename: null };
    ...
    if (destatedIds.length > 0) cleanupOrphanedTemporaryFiles(destatedIds).catch((err) => console.error(`[ImageFilterBlur Clear] Cleanup call failed:`, err));
  }, [toolState.autoSaveProcessed, toolState.selectedFileId, toolState.processedFileId, toolState.blurRadius, setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
  ...
  const initiateSave = async () => {
    ...
    if (toolState.lastUserGivenFilename && !toolState.autoSaveProcessed && processedOutputPermanent) {
      setManualSaveSuccess(true);
  ...
  if (isLoadingToolSettings && !initialToolStateLoadCompleteRef.current) {
    return <p className="text-center p-4 italic text-gray-500 animate-pulse">Loading Blur Tool...</p>;
  }
    ...
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      <div className="flex flex-col gap-3 p-3 rounded-md bg-[rgb(var(--color-bg-subtle))] border border-[rgb(var(--color-border-base))]">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <Range label="Blur Radius (px)" id="blurRadiusRange" min={0} max={25} step={1} value={toolState.blurRadius} onChange={(e) => handleBlurRadiusChange(Number(e.target.value))} disabled={isProcessingImage || isManuallySaving} />
        </div>
        <div className="flex flex-wrap gap-3 items-center pt-3 border-t border-gray-200 mt-2">
          <Checkbox label="Auto-save blurred image to Library" checked={toolState.autoSaveProcessed} onChange={handleAutoSaveChange} disabled={isProcessingImage || isManuallySaving} id="autoSaveBlurredImage" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          <label className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">Blurred Image {processedOutputPermanent && processedStoredFileForItde?.filename && <span className="font-normal text-xs">({processedStoredFileForItde.filename})</span>}</label>
          <div className="w-full aspect-square border rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
            {isProcessingImage && !processedImageSrcForUI ? <div className="flex flex-col items-center text-sm italic"><ArrowPathIcon className="animate-spin h-8 w-8 mb-2" /><p>Blurring...</p></div> : !isProcessingImage && processedImageSrcForUI ? <Image src={processedImageSrcForUI} alt={originalFilenameForDisplay ? `Blurred ${originalFilenameForDisplay}` : 'Blurred'} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} /> : !isProcessingImage && <span className="text-sm italic">Output appears here</span>}
          </div>
        </div>
      </div>
      ...
      <FilenamePromptModal isOpen={isFilenamePromptOpen} onClose={() => { setIsFilenamePromptOpen(false); setFilenamePromptAction(null); }} onConfirm={handleConfirmFilename} initialFilename={filenamePromptInitialValue} title={filenamePromptAction === 'save' ? 'Save Blurred Image to Library' : 'Download Blurred Image'} confirmButtonText={filenamePromptAction === 'save' ? 'Save to Library' : 'Download'} filenameAction={filenamePromptAction || undefined} />
    </div>
  );
}