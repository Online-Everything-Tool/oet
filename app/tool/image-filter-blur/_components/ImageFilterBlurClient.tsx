...
export default function ImageFilterBlurClient({ toolRoute }: ImageFilterBlurClientProps) {
  ...
  } = useToolState<ImageFilterBlurToolState>(toolRoute, DEFAULT_BLUR_TOOL_STATE);
  ...
  }, [originalFilenameForDisplay, processedImageSrcForUI, toolState.blurAmount, processedStoredFileForItde]);
  ...
  const blurDrawFunction = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(inputFile, blurDrawFunction, outputFileName, {}, toolState.autoSaveProcessed);
      if (result.id) {
  ...
  }, [toolState.selectedFileId, toolState.blurAmount, toolState.processedFileId, toolState.autoSaveProcessed, isLoadingToolSettings, isProcessingImage, processImage, blurDrawFunction, getFile, setState, processingErrorHook, generateDefaultOutputFilename]);
  ...
  const handleBlurAmountChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const newBlurAmount = Number(e.target.value);
    const oldProcessedId = toolState.processedFileId;

    const newState: Partial<ImageFilterBlurToolState> = {
    ...
    if (oldProcessedId) {
      cleanupOrphanedTemporaryFiles([oldProcessedId]).catch(e => console.error('[ImageFilterBlur BlurAmountChange] Cleanup call failed:', e));
    }
  ...
  const handleClear = useCallback(async () => {
    ...
    const clearedState: ImageFilterBlurToolState = {
        ...
        <div className="space-y-1">
          <label className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">
            ...
            {processedOutputPermanent && processedStoredFileForItde?.filename && (
              <span className="font-normal text-xs">({processedStoredFileForItde.filename})</span>
            )}
          </label>
          <div className="w-full aspect-square border rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
