...
export default function ImageFilterBlurClient({ toolRoute }: ImageFilterBlurClientProps) {
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const baseName = inputFile.filename?.substring(0, inputFile.filename.lastIndexOf('.')) || inputFile.filename || `image-${toolState.selectedFileId?.substring(0, 8)}`;
      const ext = inputFile.type?.split('/')[1] || 'png';
      const outputFileName = `blur-${baseName}.${ext}`;
  ...
  const handleBlurRadiusChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const newBlur = Math.max(0, Math.min(50, parseInt(e.target.value, 10)));
    setState((prev) => ({ ...prev, blurRadius: newBlur }));
    await saveStateNow({ ...toolState, blurRadius: newBlur });
  }, [toolState, setState, saveStateNow]);
    ...
    <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
      <div className="flex flex-col gap-3 p-3 rounded-md bg-[rgb(var(--color-bg-subtle))] border border-[rgb(var(--color-border-base))]">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <Range label="Blur Radius (px)" id="blurRadiusRange" min={0} max={50} step={1} value={toolState.blurRadius} onChange={handleBlurRadiusChange} disabled={isProcessingImage || isManuallySaving} containerClassName="w-full" />
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="w-full aspect-square border rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden">
            {isProcessingImage && !processedImageSrcForUI ? (
              ...
              <Image src={processedImageSrcForUI} alt={originalFilenameForDisplay ? `Blurred ${originalFilenameForDisplay}` : 'Blurred'} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} />
            ) : (
