...
const DEFAULT_TOOL_STATE: ImageFilterBlurToolState = {
  selectedFileId: null,
...
export default function ImageFilterBlurClient({ toolRoute }: ImageFilterBlurClientProps) {
  ...
  } = useToolState<ImageFilterBlurToolState>(toolRoute, DEFAULT_TOOL_STATE);
  ...
  const generateDefaultOutputFilename = useCallback(() => {
    ...
    return `blur-${baseName}-${toolState.blurAmount}.${extension}`;
      ...
      if (newSelectedFileId) {
        ...
        if (destatedIds.length > 0) {
          cleanupOrphanedTemporaryFiles(destatedIds).catch((e) =>
            console.error('[ImageFilterBlur ITDE Accept] Cleanup call failed:', e)
          );
        }
      }
    },
  ...
  const applyBlurFilter = useCallback(
  ...
  useEffect(() => {
    ...
    const triggerProcessing = async () => {
      ...
      const result = await processImage(inputFile, applyBlurFilter, outputFileName, {}, false);
  ...
  const handleClear = useCallback(async () => {
    const oldSelectedId = toolState.selectedFileId;
    const oldProcessedId = toolState.processedFileId;
    const clearedState: ImageFilterBlurToolState = { ...DEFAULT_TOOL_STATE, lastUserGivenFilename: null };
  ...
  }, [setState, saveStateNow, cleanupOrphanedTemporaryFiles, clearProcessingHookOutput]);
