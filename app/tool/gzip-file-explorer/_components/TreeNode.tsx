...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  let actionData: ActionEntryData | null = null;
  if (node.type === 'file') {
    actionData = {
      name: node.path, // full path
      id: node.id,
      _tarFile: node._tarFile,
      _decompressedBlob: node._decompressedBlob,
    };
  }
  ...
  const nodeSize = node.type === 'file' && node.size !== null ? formatBytesCompact(node.size) : '---';
    ...
    <div>
      ...
      {isExpanded && node.children && node.children.length > 0 && (
        <div className="border-l border-[rgba(var(--color-border-base)/0.3)] ml-[calc(0.8rem+2px)]">
          {node.children
            .map((childNode) => ( // No sort here, assuming utils.ts already sorted
              <TreeNode
                key={childNode.id}
                node={childNode}
                level={level + 1}
