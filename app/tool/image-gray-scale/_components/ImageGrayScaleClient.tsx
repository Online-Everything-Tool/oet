...
export default function ImageGrayScaleClient({ toolTitle, toolRoute }: ImageGrayScaleClientProps) {
    ...
    const convertToGrayScale = useCallback((ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
        ...
        for (let i = 0; i < data.length; i += 4) {
            ...
            data[i] = luminance; data[i + 1] = luminance; data[i + 2] = luminance;
        }
    ...
    useEffect(() => {
        ...
        if (selectedFile?.blob) {
            try {
                objectUrl = URL.createObjectURL(selectedFile.blob); setOriginalImageSrc(objectUrl); setFileName(selectedFile.name);
                setProcessedImageSrc(null); setError(null); setLastProcessedImageId(null); setIsCopied(false);
            } catch (e) { console.error("Error creating object URL:", e); setError("Could not create preview."); setOriginalImageSrc(null); setFileName(null); }
        } else { setOriginalImageSrc(null); setFileName(null); }
    ...
    useEffect(() => {
        if (selectedFile && originalImageSrc) {
            const triggerProcessing = async () => {
                ...
                const newImageId = await processImage( selectedFile, convertToGrayScale, 'auto', outputFileName );
    ...
    }, [setError]); // Dependency remains setError

    // Other handlers remain the same
    const handleClear = useCallback(() => { setOriginalImageSrc(null); setProcessedImageSrc(null); setFileName(null); setError(null); setIsLoading(false); setSelectedFile(null); setLastProcessedImageId(null); setIsCopied(false); }, [setOriginalImageSrc, setProcessedImageSrc, setFileName, setError, setIsLoading]);
    const handleDownload = useCallback(() => { if (!processedImageSrc || !fileName) { setError('No processed image available to download.'); return; } setError(null); const link = document.createElement('a'); const baseName = fileName.substring(0, fileName.lastIndexOf('.')) || fileName; const extension = fileName.substring(fileName.lastIndexOf('.') + 1) || 'png'; link.download = `grayscale-${baseName}.${extension}`; link.href = processedImageSrc; document.body.appendChild(link); link.click(); document.body.removeChild(link); }, [processedImageSrc, fileName, setError]);
    const handleCopyToClipboard = useCallback(async () => { if (!lastProcessedImageId) { setError('No processed image ID to copy.'); return; } setIsCopied(false); setError(null); try { const imageData = await getImage(lastProcessedImageId); if (!imageData?.blob) throw new Error("Processed image blob not found."); if (!navigator.clipboard?.write) throw new Error("Clipboard API not available."); const clipboardItem = new ClipboardItem({ [imageData.blob.type]: imageData.blob }); await navigator.clipboard.write([clipboardItem]); setIsCopied(true); setTimeout(() => setIsCopied(false), 2000); } catch (err) { console.error('Failed to copy image:', err); const message = err instanceof Error ? err.message : 'Unknown error'; setError(`Copy failed: ${message}`); } }, [lastProcessedImageId, getImage, setError]);
        ...
        <div className="flex flex-col gap-5 text-[rgb(var(--color-text-base))]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="space-y-1">
                    ...
                    <div className="w-full aspect-square border border-[rgb(var(--color-input-border))] rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden"> {originalImageSrc ? ( <Image src={originalImageSrc} alt={fileName || "Original"} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} /> ) : (<span className="text-sm text-[rgb(var(--color-input-placeholder))] italic">Select an image from library</span>)} </div>
                </div>
                 <div className="space-y-1">
                    <label className="block text-sm font-medium text-[rgb(var(--color-text-muted))]">Grayscale Image</label>
                    <div className="w-full aspect-square border border-[rgb(var(--color-input-border))] rounded-md bg-[rgb(var(--color-bg-subtle))] flex items-center justify-center overflow-hidden"> {isLoading && !processedImageSrc && (<span className="text-sm text-[rgb(var(--color-text-link))] italic animate-pulse">Converting...</span>)} {!isLoading && processedImageSrc ? (<Image src={processedImageSrc} alt={fileName ? `Grayscale ${fileName}` : "Grayscale Image"} width={500} height={500} className="max-w-full max-h-full object-contain" unoptimized={true} />) : !isLoading && (<span className="text-sm text-[rgb(var(--color-input-placeholder))] italic">Output appears here</span>)} </div>
                </div>
            </div>
            {/* File Selection Modal */}
            <FileSelectionModal isOpen={isLibraryModalOpen} onClose={() => setIsLibraryModalOpen(false)} onFilesSelected={handleFilesSelected} libraryFilter={{ category: 'image' }} selectionMode="single" accept="image/*" className="max-w-4xl" />
        </div>
    );
}