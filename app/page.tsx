...
async function getProjectAnalysisData(): Promise<ProjectAnalysisData | null> {
  ...
  try {
    ...
    if (
      ...
      data.suggestedNewToolDirectives &&
      data.siteTagline
    ) {
      return data;
    } else {
      console.warn(
        '[Page Load] project_analysis.json might be missing some fields (description, suggestions, tagline).'
      );
    ...
    if (errorCode === 'ENOENT') {
      console.log(
        '[Page Load] project_analysis.json not found. Displaying default content.'
      );
    } else {
      console.error(
        '[Page Load] Error reading or parsing project_analysis.json:',
        message
      );
    }
    return null;
      ...
      <div className="text-center border-b border-[rgb(var(--color-border-base))] pb-6 mb-6">
        ...
        {analysisData?.modelNameUsed && (
          <p className="text-xs text-gray-400 mt-2">
            Analysis powered by {analysisData.modelNameUsed}
          </p>
        )}
      </div>
