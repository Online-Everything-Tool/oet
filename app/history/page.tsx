...
export default function HistoryPage() {
  ...
  const handleReload = async (entry: HistoryEntry) => {
    if (!entry.toolRoute || !entry.toolRoute.startsWith('/tool/')) {
      console.error(
        '[Reload] Invalid toolRoute in history entry:',
        entry.toolRoute
      );
      alert('Cannot reload: Invalid tool route found in history.');
      return;
    }
    const directiveName = entry.toolRoute
      .substring('/tool/'.length)
      .replace(/\/$/, '');
    if (!directiveName) {
      console.error(
        '[Reload] Could not extract directive name from route:',
        entry.toolRoute
      );
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            `Metadata file not found for tool '${directiveName}'.`
          );
        }
    ...
    if (typeof entry.input !== 'object' || entry.input === null) {
      alert(
        `Cannot reload: History entry has unexpected input format for this tool's URL parameters.`
      );
      return;
    }
        ...
        if (value !== undefined) {
          ...
          if (value === null) {
            ...
            try {
              ...
              console.error(
                `[Reload] Error stringifying value for param '${paramNameKey}':`,
                value,
                e
              );
              valueString = '';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-center border-b pb-2 mb-4 gap-4">
        ...
        <div className="flex items-center gap-2 bg-gray-100 border border-gray-300 p-2 rounded-md">
          <label
            ...
            className="w-10 text-sm font-medium text-gray-700 cursor-pointer select-none text-center"
          >
            {' '}
            Log{' '}
          </label>
          ...
          />
          <span
            className={`text-sm w-15 font-semibold ${isHistoryEnabled ? 'text-green-700' : 'text-red-700'}`}
          >
        ...
        <button
          ...
          className="px-4 py-2 rounded-md text-sm font-medium bg-[rgb(var(--color-button-danger-bg))] text-[rgb(var(--color-button-danger-text))] hover:bg-[rgb(var(--color-button-danger-hover-bg))] focus:outline-none focus:ring-offset-2 disabled:bg-gray-300 disabled:cursor-not-allowed transition duration-150 ease-in-out"
        >
          Clear All History
        </button>
      </div>
      ...
      {isLoaded && !isHistoryEnabled && history.length > 0 && (
        <p className="p-3 bg-yellow-100 border border-yellow-300 text-yellow-800 rounded-md text-sm">
          History logging is currently disabled. Existing entries are shown
          below, but new actions will not be recorded until enabled.
        </p>
      )}
        ...
        <ul className="space-y-4">
          ...
          {history
            ...
            .map((entry: HistoryEntry) => {
              const isToolPage =
                entry.toolRoute && entry.toolRoute.startsWith('/tool/');

              return (
                <li
                  key={entry.id}
                  className="p-4 border rounded-md shadow-sm bg-white flex justify-between items-start gap-4"
                >
                  <div className="flex-grow overflow-hidden space-y-1">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <p
                        ...
                        title={`Event occurred at: ${new Date(entry.eventTimestamp).toLocaleString()}`}
                      >
                        {formatEventTimestamp(entry.eventTimestamp)}{' '}
                        {/* Display formatted event time */}
                      </p>
                    </div>
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      <code className="text-xs bg-gray-100 px-1 rounded">
                        {formatTriggerType(entry.trigger)}
                      </code>{' '}
                      |{' '}
                      <span
                        className={`font-medium ${entry.status === 'error' ? 'text-red-600' : 'text-green-600'}`}
                      >
                        {' '}
                    ...
                    {entry.output !== undefined && entry.output !== null && (
                      <div>
                        <span
                          className={`text-xs font-medium ${entry.status === 'error' ? 'text-red-500' : 'text-gray-500'}`}
                        >
                          {entry.status === 'error'
                            ? 'Error Output:'
                            : 'Output:'}
                        ...
                        <pre
                          className={`mt-1 p-2 rounded text-xs overflow-x-auto whitespace-pre-wrap break-words max-h-24 ${entry.status === 'error' ? 'bg-red-50 text-red-700' : 'bg-gray-50 text-gray-600'}`}
                        >
                          {typeof entry.output === 'object'
                            ? JSON.stringify(entry.output, null, 2)
                            : String(entry.output).substring(0, 500) +
                              (String(entry.output).length > 500 ? '...' : '')}
                        </pre>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    <button
                      ...
                      title={`Reload ${entry.toolName} with this state`}
                      className="px-3 py-1 rounded text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={!isToolPage}
                    >
                      {' '}
                      Reload{' '}
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded text-xs font-medium text-red-600 hover:bg-red-100 focus:outline-none"
                    >
                      {' '}
                      Delete{' '}
                    </button>
