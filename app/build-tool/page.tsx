...
export default function BuildToolPage() {
  ...
  const [prSubmissionResult, setPrSubmissionResult] =
    useState<PrSubmissionResult | null>(null);
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            ...
            const specificMsg =
              'The AI model listing API (/api/list-models) was not found. This feature requires server functionality.';
        ...
        const defaultModel =
          models.find((m) => m.name === defaultEnv) ??
        ...
        setIsApiUnavailable(true);
        const specificMsg =
          'Failed to connect to the AI model API. This feature may require server functionality.';
        setApiUnavailableMessage(specificMsg);
        setModelsError(specificMsg);
        setAvailableModels([]);
      } finally {
    ...
    const fetchDirectives = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error(
              'Directives list file (/api/directives.json) not found. Run the build script.'
            );
          }
          throw new Error(
            `Failed to fetch directives list (${response.status})`
          );
        }
  ...
  const handlePrSubmissionSuccess = useCallback(
    (result: PrSubmissionResult) => {
      ...
      return (
        <div className="p-6 border rounded-lg bg-orange-50 border-orange-300 shadow-sm text-center">
          ...
          <p className="text-orange-700 mb-4">
            The AI-assisted build tool relies on server-side APIs which are not
            available in this static deployment environment.
          </p>
          ...
          <p className="text-sm text-gray-600">
            To contribute a new tool, please follow the manual setup
            instructions in the{' '}
            <Link
    ...
    switch (currentStep) {
      ...
      case 'submission':
        ...
        if (
          ...
          console.error(
            'Missing data required for submission step, resetting.'
          );
          handleReset();
          return null;
        }
      ...
      default:
        return (
          <p className="text-center text-red-500">Error: Invalid build step.</p>
        );
    }
  };
