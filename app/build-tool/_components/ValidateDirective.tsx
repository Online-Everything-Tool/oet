...
export default function ValidateDirective({
    toolDirective, setToolDirective,
    selectedModel, setSelectedModel,
    availableModels, modelsLoading, modelsError,
    onValidationSuccess,
    // onReset, // Include if used
}: ValidateDirectiveProps) { // Use renamed Props Interface
    ...
    const handleValidateClick = async () => {
        ...
        if (!finalDirective) { setStatus('error'); setFeedback('Please enter a valid tool directive.'); setIsValidating(false); return; }
        if (!selectedModel) { setStatus('error'); setFeedback('Please select an AI model.'); setIsValidating(false); return; }
        ...
        try {
            ...
            if (data.generativeDescription) {
                ...
                onValidationSuccess({
                    ...
                    generativeRequestedDirectives: data.generativeRequestedDirectives || []
                });
                setFeedback(null);
                setStatus('idle');
            } else {
    ...
    return (
        <section className={`p-4 border rounded-lg bg-white shadow-sm transition-opacity duration-300 ${isValidating ? 'opacity-70' : ''} ${status === 'error' ? 'border-red-300' : 'border-gray-200'}`}>
            {/* Title reflects the step/purpose */}
            <h2 className="text-lg font-semibold mb-3 text-gray-700">Step 1: Validate Directive</h2>
            <p className="text-sm text-gray-600 mb-4">
                Enter a unique, URL-friendly directive and select the AI model for validation.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                 <div>
                    ...
                    <input type="text" id="toolDirective" value={toolDirective} onChange={(e) => setToolDirective(e.target.value)}
                    ...
                    />
                     <p className="mt-1 text-xs text-gray-500">URL: `/tool/{formatSlug(toolDirective) || '...'}`</p>
                </div>
                 <div>
                    ...
                    {modelsLoading && <p className="text-sm text-gray-500 animate-pulse h-9 flex items-center">Loading...</p>}
                    {modelsError && <p className="text-sm text-red-600 h-9 flex items-center">Error: {modelsError}</p>}
                    {!modelsLoading && !modelsError && availableModels.length === 0 && <p className="text-sm text-orange-600 h-9 flex items-center">No models.</p>}
                    {!modelsLoading && !modelsError && availableModels.length > 0 && (
                        <select id="aiModelSelect" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)}
                            disabled={isValidating || modelsLoading || availableModels.length === 0}
                        ...
                        >
                            {selectedModel === '' && <option value="" disabled>-- Select --</option>}
                            {availableModels.map(model => <option key={model.name} value={model.name}>{model.displayName} ({model.name.replace('models/', '')})</option>)}
                        </select>
                    )}
                     <p className="mt-1 text-xs text-gray-500">Model used for validation & generation.</p>
                </div>
            </div>

            {/* Button to trigger the validation */}
            <button
                type="button" onClick={handleValidateClick}
                disabled={isValidating || !toolDirective.trim() || modelsLoading || !selectedModel}
