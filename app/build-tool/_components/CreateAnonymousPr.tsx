...
export default function CreateAnonymousPr({
  ...
  const handleAnonymousSubmitClick = async () => {
    ...
    if (!toolDirective) {
      setStatus('error');
      setFeedback('Error: Tool directive is missing.');
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unexpected error during PR submission.';
  ...
  const toggleFileExpansion = (filePath: string) => {
    setExpandedFilePath((currentPath) =>
      currentPath === filePath ? null : filePath
    );
  };
    ...
    >
      ...
      <div className="mb-4 space-y-3 p-3 bg-gray-50 border border-gray-200 rounded">
                ...
                {validationResult.generativeRequestedDirectives.map((d) => (
                  <li key={d} className="text-sm text-indigo-700">
                    <Link
                      ...
                      className="underline hover:text-indigo-900 font-mono text-xs"
                    >
                      {d}
                    </Link>
                  </li>
        ...
        <div>
          ...
          {userSelectedDirectives && userSelectedDirectives.length > 0 ? (
            <ul className="list-disc list-inside space-y-1 pl-2">
              {userSelectedDirectives.map((d) => (
                <li key={d} className="text-sm text-purple-700">
                  <Link
                    href={`/tool/${d}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline hover:text-purple-900 font-mono text-xs"
                  >
                    {d}
                  </Link>
                </li>
        ...
        <div className="pt-2 border-t border-gray-200 mt-2">
          ...
          generationResult.identifiedDependencies.length > 0 ? (
            <ul className="list-disc list-inside space-y-1 pl-2">
              {generationResult.identifiedDependencies.map(
          ...
          {generationResult.identifiedDependencies &&
            generationResult.identifiedDependencies.length > 0 && (
              <p className="text-xs text-gray-500 mt-1">
                Note: These may need to be added to `package.json` manually if
                not already present.
              </p>
            )}
        </div>
      ...
      <div className="mb-4">
        ...
        <p className="mt-2 text-xs text-gray-500">
          Click file names to expand/collapse content. Review before submitting.
        </p>
      </div>
