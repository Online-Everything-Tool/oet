...
// Import shared types
import type { GenerationResult, ValidationResult, PrSubmissionResult, LibraryDependency, ApiPrSubmissionResponseData } from '@/src/types/build';
...
export default function CreateAnonymousPr({
    ...
    onBack
}: CreateAnonymousPrProps) { // Use updated props type
    ...
    const handleAnonymousSubmitClick = async () => {
        ...
        if (!selectedModel) {
             setStatus('error'); setFeedback('Error: AI Model selection information is missing.'); return;
        }
    ...
    return (
         <section className={`p-4 border rounded-lg bg-white shadow-sm transition-opacity duration-300 ${isSubmitting ? 'opacity-70' : ''} ${status === 'error' ? 'border-red-300' : 'border-purple-300'}`}>
            ...
            <div className="mb-4 space-y-3 p-3 bg-gray-50 border border-gray-200 rounded">
                 ...
                 <div>
                     <p className="text-sm font-medium text-gray-600 mb-1">User Selected Examples:</p>
                         ...
                         <ul className="list-disc list-inside space-y-1 pl-2">
                             {userSelectedDirectives.map(d => (
                                 <li key={d} className="text-sm text-purple-700">
                                     <Link href={`/tool/${d}`} target="_blank" rel="noopener noreferrer" className="underline hover:text-purple-900 font-mono text-xs">
                                         {d}
                                     </Link>
                                 </li>
                             ))}
